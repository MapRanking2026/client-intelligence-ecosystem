 ...
 * to (a second, named Admin app) — so MTOS and SEOOS read the same canonical store
 * even though each keeps its own project for its own data. When it isn't set, this
 * falls back to MTOS's own Firestore, so nothing breaks before the env is configured.
 */
...
export function getEngineFirebaseDb(): Firestore | null {
  ...
  // No dedicated engine project configured → use MTOS's own Firestore.
