...
import { resetRule, upsertRule } from "@/src/lib/server/rules-service";
...
export async function PUT(
  ...
  try {
    ...
    if (!body?.value?.trim()) return NextResponse.json({ error: "Value is required." }, { status: 400 });
    const saved = await upsertRule(authz.tenantId, key, body.value, authz.userId);
    return NextResponse.json({ data: { key: saved.key, value: saved.value, updatedAt: saved.updatedAt } });
  } catch (e) {
...
export async function DELETE(
  ...
  try {
    adminGuard(authz);
    const { key } = await params;
    await resetRule(authz.tenantId, key);
