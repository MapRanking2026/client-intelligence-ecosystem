...
| Native Rank Tracker + Map Check-Ins (MapRanking dashboard) | verified | `lib/server/sync/mapranking-client.ts` copies MTOS's dashboard client (email/password → bearer, auto-refresh per sync); one connection powers rankings, grids, and check-ins; wired into source-sync (native, no gateway); catalog `rank-tracker` (client id/secret/base url) + `map-checkins` poweredBy |
